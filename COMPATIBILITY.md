...
Source placement remains strict by default. Exact alternate source ownership and explicit compatibility placement are additive request fields. Compatibility is admitted only for a sole `DOTNETSP0004` insufficient-structure result; other diagnostics retain their published meaning and remain blocking. Placement results carry the exact strict and compatibility policy data used for deterministic host provenance.
